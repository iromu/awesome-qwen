...
| [embabel-chatbot](skills/embabel-chatbot) | Build agentic chatbots with tool-based RAG — `Chatbot`/`ChatSession`, `@Action`-triggered replies, `ToolishRag` grounding, and durable conversations via `embabel-chat-store` | `chatbot` `rag` `jvm` |
